...
def play():
    def get_guess(user_guess):
        """
        This function starts each move to check if the game is won,
        lost, needs to move on to move check or if the input is a valid type
        :param user_guess: needed to bind Tk list
        """
        if winner.get():
            new_game(rng.get())
        else:
            try:
                ...
                user_guess = int(guess_.get()) if user_guess != guess_.get() else int(user_guess)
                secret_number = secret.get()
                print(f'guess: {user_guess}\t secret: {secret_number}')  # print current Secret #
    ...
    def won_game():
        ...
        while winner:
            color = choice(colors)
            win_msg = (f'{user_guess}-' * 3 + f'{user_guess}\n') * 2
            guessed.config(text=win_msg, foreground=color)
            game_msg.config(text=f'{user_guess} was the secret number!\nTries: {tries.get()}',
                            foreground=color)
            win.update()
            time.sleep(.1)
            if not winner.get():
    ...
    def game_over():
        guessed.config(text=f'Sorry,You\'ve lost.\n"{secret.get()}" was the SECRET number.')
        game_msg.config(text='All 15 tries were used\nTime to play again!', foreground='royal blue')
        win.update()
        time.sleep(3)
        new_game(rng.get())
    ...
    def quit_game():
        if winner.get():
            wins.set(wins.get() + 1)
        winner.set(False)
        goodbye = f'You played: {games.get()} games\nYou won: {wins.get()}'
        guessed.config(text='Goodbye,\nThank you for playing!!!', foreground='royal blue')
        game_msg.config(text=goodbye, foreground='royal blue')
        win.update()
        time.sleep(1.5)
        exit()
    ...
    quit_ = ttk.Button(win, text='Quit', command=quit_game)
